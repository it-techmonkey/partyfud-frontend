...
interface OrdersListProps {
    catererId?: string; // Optional prop to filter by caterer
}

export default function OrdersList({ catererId }: OrdersListProps) {
    ...
    const [catererFilter, setCatererFilter] = useState(catererId || 'ALL'); // Initialize with prop if provided
    ...
    useEffect(() => {
        // Only fetch caterers if we are not in single caterer mode
        if (!catererId) {
            fetchCaterers();
        } else {
            // Even if catererId is passed, we might want to ensure the filter stays set to it
            setCatererFilter(catererId);
        }
    }, [catererId]);
    ...
    const fetchOrders = async () => {
        ...
        try {
            ...
            // If the component has a fixed catererId, use it. Otherwise use the filter state.
            const currentCatererFilter = catererId || catererFilter;

            if (currentCatererFilter !== 'ALL') {
                // Important: backend expects 'catererId'
                // However, for the dropdown state `catererFilter` holds the ID
                // And if passed `catererId` prop, we use that.
                params.catererId = currentCatererFilter;
            }
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                    ...
                    {/* Caterer Filter - Only show if not filtering by specific caterer */}
                    {!catererId && (
                        <div>
